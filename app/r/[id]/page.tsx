...
async function getData(name: string) {
  noStore();
  const data = await prisma.subreadit.findUnique({
    where: {
      name: name,
    },
...
export default async function SubReaditRoute({
  ...
  return (
    <div className="max-w-[1000px] mx-auto flex gap-x-10 mt-4 mb-10 px-4">
          ...
          <div className="p-4">
            ...
            <div className="flex items-center gap-x-2 mt-4">
              ...
              <p className="text-muted-foreground font-medium text-sm">
                Created:{" "}
                {new Date(data?.createdAt as Date).toLocaleDateString("id-ID", {
                  weekday: "long",
                  year: "numeric",
                  month: "short",
                  day: "numeric",
